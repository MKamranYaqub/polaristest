...
const BridgingDIPPDF = ({ quote, dipData, brokerSettings = {} }) => {
  ...
  // Financial Covenants - pull from quote data (saved from selected rate)
  // Fallback: use calculated LTV for maxLTV, 110% for minICR if not in quote
  const maxLTV = h.parseNumber(quote.max_ltv) || parseFloat(ltv) || 75;
  const minICR = h.parseNumber(quote.min_icr) || 110;
...
{/* Title Insurance - CONDITIONAL on title_insurance === 'Yes' */}
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={btlDipStyles.termsSection}>
          ...
          {isVariableRate && (
            <Text style={btlDipStyles.termsText}>
              If BBR is 0.5% per annum or less, then, for the purposes of calculating your relevant rate, we will treat BBR as 0.5% per annum and apply any margin to that.
            </Text>
          )}
        </View>
        ...
        <View style={btlDipStyles.termsSection}>
          ...
          <Text style={btlDipStyles.termsText}>
            The Loan shall be for the Loan Term, which will commence from the date of drawdown of the Loan.  Please note that this is an interest only BTL Loan facility, and you must be in a position to repay the capital in full at the end of the Term. You should not assume that the Loan Term will be extended.
          </Text>
        </View>
